      ...
      <div style={{ background: "#faf7f2", border: "1px solid #e0d9ce", borderRadius: 20, padding: "40px 36px", width: 320, maxWidth: "90vw", boxShadow: "0 8px 32px rgba(42,32,24,.1)", textAlign: "center" }}>
        ...
        <div style={{ marginBottom: 16 }}>
          <input
            ...
            style={{
              width: "100%", background: err ? "#fdf0ee" : "#f5f0e8",
              border: `1.5px solid ${err ? "#c0392b" : "#ccc5ba"}`,
              borderRadius: 10, padding: "12px 14px", color: "#2a2018",
              fontFamily: "Noto Sans JP,sans-serif", fontSize: 16,
              outline: "none", textAlign: "center", letterSpacing: "0.2em",
              transition: "border-color .2s, background .2s",
            }}
            autoFocus
          />
          {err && <p style={{ color: "#c0392b", fontSize: 12, marginTop: 8, fontFamily: "Noto Sans JP,sans-serif" }}>パスワードが違います</p>}
        </div>
        <button className="pbtn" style={{ width: "100%", padding: "12px" }} onClick={handleLogin}>
          入る
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  const [rnCat, setRnCat] = useState(null);
  const [rnCatV, setRnCatV] = useState("");
  const [rnBrand, setRnBrand] = useState(null);
  const [rnBrandV, setRnBrandV] = useState("");
  const [rnItem, setRnItem] = useState(null);
  const [rnItemV, setRnItemV] = useState("");
  ...
  return <MainApp
    cats={cats} setCats={setCats} loading={loading} setLoading={setLoading}
    saving={saving} setSaving={setSaving} tab={tab} setTab={setTab}
    adjModal={adjModal} setAdjModal={setAdjModal}
    addItemModal={addItemModal} setAddItemModal={setAddItemModal}
    addBrandModal={addBrandModal} setAddBrandModal={setAddBrandModal}
    addCatModal={addCatModal} setAddCatModal={setAddCatModal}
    minModal={minModal} setMinModal={setMinModal}
    stOpen={stOpen} setStOpen={setStOpen} stTab={stTab} setStTab={setStTab}
    stCatId={stCatId} setStCatId={setStCatId} stBrandId={stBrandId} setStBrandId={setStBrandId}
    rnCat={rnCat} setRnCat={setRnCat} rnCatV={rnCatV} setRnCatV={setRnCatV}
    rnBrand={rnBrand} setRnBrand={setRnBrand} rnBrandV={rnBrandV} setRnBrandV={setRnBrandV}
    rnItem={rnItem} setRnItem={setRnItem} rnItemV={rnItemV} setRnItemV={setRnItemV}
    newItemF={newItemF} setNewItemF={setNewItemF}
    newBrandF={newBrandF} setNewBrandF={setNewBrandF}
    newCatF={newCatF} setNewCatF={setNewCatF}
    adjVal={adjVal} setAdjVal={setAdjVal} minVal={minVal} setMinVal={setMinVal}
  />;
}

function MainApp(props) {
  const {
    cats, setCats, loading, setLoading, saving, setSaving, tab, setTab,
    adjModal, setAdjModal, addItemModal, setAddItemModal,
    addBrandModal, setAddBrandModal, addCatModal, setAddCatModal,
    minModal, setMinModal, stOpen, setStOpen, stTab, setStTab,
    stCatId, setStCatId, stBrandId, setStBrandId,
    rnCat, setRnCat, rnCatV, setRnCatV,
    rnBrand, setRnBrand, rnBrandV, setRnBrandV,
    rnItem, setRnItem, rnItemV, setRnItemV,
    newItemF, setNewItemF, newBrandF, setNewBrandF, newCatF, setNewCatF,
    adjVal, setAdjVal, minVal, setMinVal,
  } = props;
  ...
  const doAdj = async () => {
    ...
    const sid = adjModal.itemId;
    setAdjModal(null); setAdjVal("");
    setSaving(true);
    await api(`items?id=eq.${sid}`, "PATCH", { stock: newStock });
    setSaving(false);
  };
  ...
  const doAddBrand = async () => {
    ...
    const newId = uid();
    const newBrand = { id: newId, category_id: catId, name: newBrandF.trim(), order: maxOrd + 1, items: [] };
    setCats(p => p.map(c => c.id !== catId ? c : { ...c, brands: [...(c.brands || []), newBrand] }));
    setNewBrandF(""); setAddBrandModal(null);
    setSaving(true);
    await api("brands", "POST", { id: newId, category_id: catId, name: newBrand.name, order: newBrand.order });
    setSaving(false);
  };
  ...
  const doAddCat = async () => {
    ...
    setSaving(true);
    await api("categories", "POST", { id: newId, name: newCatF.trim(), order: maxOrd + 1 });
    setSaving(false);
  };
  ...
  const doMin = async () => {
    ...
    const sid = minModal.itemId;
    setMinModal(null); setMinVal("");
    setSaving(true);
    await api(`items?id=eq.${sid}`, "PATCH", { min_stock: n });
    setSaving(false);
  };
  ...
  const moveCat = async (catId, dir) => {
    ...
    await Promise.all([api(`categories?id=eq.${a[idx].id}`, "PATCH", { order: a[idx].order }), api(`categories?id=eq.${a[sw].id}`, "PATCH", { order: a[sw].order })]);
    setSaving(false);
  };
  ...
  const moveBrand = async (catId, brandId, dir) => {
    ...
    await Promise.all([api(`brands?id=eq.${a[idx].id}`, "PATCH", { order: a[idx].order }), api(`brands?id=eq.${a[sw].id}`, "PATCH", { order: a[sw].order })]);
    setSaving(false);
  };
  ...
  const moveItem = async (catId, brandId, itemId, dir) => {
    ...
    setSaving(true);
    await Promise.all([api(`items?id=eq.${a[idx].id}`, "PATCH", { order: a[idx].order }), api(`items?id=eq.${a[sw].id}`, "PATCH", { order: a[sw].order })]);
    setSaving(false);
  };
  ...
  const commitRnCat = async (catId) => {
    ...
    setRnCat(null); setSaving(true);
    await api(`categories?id=eq.${catId}`, "PATCH", { name: rnCatV.trim() }); setSaving(false);
  };
  ...
  const commitRnBrand = async (catId, brandId) => {
    ...
    setRnBrand(null); setSaving(true);
    await api(`brands?id=eq.${brandId}`, "PATCH", { name: rnBrandV.trim() }); setSaving(false);
  };
  ...
  const commitRnItem = async (catId, brandId, itemId) => {
    ...
    setRnItem(null); setSaving(true);
    await api(`items?id=eq.${itemId}`, "PATCH", { name: rnItemV.trim() }); setSaving(false);
  };
    ...
    <div style={S.root}>
      ...
      <header style={S.hdr}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <button className="icobtn" onClick={loadData} title="更新"><Ico.Refresh /></button>
      ...
      <main style={S.main}>
        {tab === "order" && (
          needOrder.length === 0
            ...
            : <>
              <p style={{ color: "#c0392b", fontFamily: "Syne,sans-serif", fontWeight: 700, marginBottom: 16, fontSize: 14 }}>
                ⚠️ {needOrder.length}点が注文ラインに達しています
              </p>
              {needOrder.map(i => (
                        ...
                        <div key={item.id} className="irow">
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ display: "flex", gap: 10, marginTop: 3, flexWrap: "wrap" }}>
                              ...
                              <span className="minlink" onClick={() => { setMinModal({ catId: cat.id, brandId: brand.id, itemId: item.id }); setMinVal(String(item.minStock)); }}>
                                注文ライン: {item.minStock}
                              </span>
                            </div>
                          </div>
        ...
        <div className="stover" onClick={() => setStOpen(false)}>
          <aside className="stpanel" onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              <span style={{ fontFamily: "Syne,sans-serif", fontWeight: 800, fontSize: 17, color: "#2a2018", display: "flex", alignItems: "center", gap: 7 }}>
                <Ico.Settings /> 設定・並び替え
              </span>
            ...
            <div style={{ display: "flex", gap: 4, marginBottom: 18, background: "#ede8df", borderRadius: 9, padding: 4 }}>
              {["cats", "brands", "items"].map(t => (
                <button key={t} className={`sttab ${stTab === t ? "sttabon" : ""}`}
                  onClick={() => { setStTab(t); setRnCat(null); setRnBrand(null); setRnItem(null); }}>
              ...
              <>
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {sortedCats.map(c => (
                      <button key={c.id} className={`chip ${stCatId === c.id ? "chipon" : ""}`} onClick={() => { setStCatId(c.id); setRnBrand(null); }}>{c.name}</button>
                    ))}
                  </div>
                </div>
                {stCatId ? (
              ...
              <>
                <div style={{ marginBottom: 10 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 10 }}>
                    {sortedCats.map(c => (
                      <button key={c.id} className={`chip ${stCatId === c.id ? "chipon" : ""}`} onClick={() => { setStCatId(c.id); setStBrandId(null); setRnItem(null); }}>{c.name}</button>
                    ))}
                  </div>
                  {stCatId && (
                    <>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                        {sortedStBrands.map(b => (
                          <button key={b.id} className={`chip ${stBrandId === b.id ? "chipon" : ""}`} onClick={() => { setStBrandId(b.id); setRnItem(null); }}>{b.name}</button>
                        ))}
                      </div>
                    </>
                  )}
        ...
        return (
          <div className="mover" onClick={() => setAdjModal(null)}>
            <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              <div className="fg">
                <label>{adjModal.mode === "add" ? "追加する数量" : "減らす数量"}</label>
                <input type="number" min="0" value={adjVal} onChange={e => setAdjVal(e.target.value)} placeholder="数量を入力" autoFocus onKeyDown={e => e.key === "Enter" && doAdj()} />
              </div>
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .smbtn.brand { background: #d6e4f0; color: #2563a8; }
  .smbtn.brand:hover { background: #c4d6ea; }
  ...
  .ton { background: #2a2018; color: #f5f0e8; }
  .toff { color: #9a8f82; }
  .toff:hover { color: #2a2018; background: #e8e2d8; }
  .tred { background: #c0392b; color: #fff; }
  ...
  .minlink { color: #b0a898; font-size: 11px; cursor: pointer; text-decoration: underline; }
  .minlink:hover { color: #7a6f63; }
  ...
  .fg input:focus, .fg select:focus { border-color: #2a2018; }
  ...
  .strow:hover { border-color: #c8bfb0; }
  ...
  .chip:hover { background: #ddd6ca; color: #2a2018; }
