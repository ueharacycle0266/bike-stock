      ...
      <PageWrap mode={mode} switchMode={switchMode}>
        ...
        <Modal open={!!editSlotModal} onClose={()=>setEditSlotModal(null)} title={`スロット ${editSlotModal?.slot_no}`}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:4}}>
            ...
            <div style={{minWidth:0}}><FG label="引取予定日"><CInput type="date" value={editSlotModal?.pickup||""} onChange={v=>setEditSlotModal(p=>({...p,pickup:v}))}/></FG></div>
          </div>
          {/* 自転車ごとの修理内容 */}
              ...
              <div key={bi} style={{marginBottom:12,background:"#faf8f4",borderRadius:10,padding:"10px 12px",border:"1px solid rgba(42,32,24,.08)"}}>
                ...
                {(bike.items||[]).map((it,idx)=>(
                  <div key={idx} style={{display:"grid",gridTemplateColumns:"1fr 36px 72px 24px 20px",gap:4,marginBottom:5,alignItems:"center"}}>
                    <input value={it.name||""} onChange={e=>{ const v=e.target.value; const menu=repairMenus.find(m=>m.name===v); updBike(b=>({...b,items:b.items.map((x,i)=>i===idx?{...x,name:v,...(menu&&!x.price?{price:String(menu.price)}:{})}:x)})); }} placeholder="品名" style={{background:"#fff",border:"1px solid rgba(42,32,24,.1)",borderRadius:6,padding:"6px 8px",fontSize:12,color:it.name?"#2a2018":"#9a9088",fontFamily:"'Noto Sans JP',sans-serif",outline:"none",minWidth:0,width:"100%"}}/>
                    <input type="number" value={it.qty||""} onChange={e=>updBike(b=>({...b,items:b.items.map((x,i)=>i===idx?{...x,qty:e.target.value}:x)}))} placeholder="数" style={{background:"#fff",border:"1px solid rgba(42,32,24,.1)",borderRadius:6,padding:"6px 4px",fontSize:12,color:"#2a2018",textAlign:"center",outline:"none",minWidth:0}}/>
                    <input type="number" value={it.price||""} onChange={e=>updBike(b=>({...b,items:b.items.map((x,i)=>i===idx?{...x,price:e.target.value}:x)}))} placeholder="単価" style={{background:"#fff",border:"1px solid rgba(42,32,24,.1)",borderRadius:6,padding:"6px 6px",fontSize:12,color:"#2a2018",textAlign:"right",outline:"none",minWidth:0,fontFamily:"'DM Mono',monospace"}}/>
                    ...
                    <button onClick={()=>updBike(b=>({...b,items:b.items.filter((_,i)=>i!==idx)}))} style={{background:"none",border:"none",cursor:"pointer",color:"#c8bfb0",display:"flex",alignItems:"center",justifyContent:"center",padding:0}}><Ico.Trash/></button>
                  </div>
                ))}
